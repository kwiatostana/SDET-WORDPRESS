...
class TestPostsD1:
    ...
    def test_create_post(self, api_client: APIClient, db_client: DBClient, cleanup_posts):
        ...
        with allure.step("Проверить что POST /wp/v2/posts вернул 201 и корректный payload"):
            assert data["title"]["raw"] == title, (
                f"Заголовок поста не совпадает. Ожидалось: '{title}', получено: '{data['title']['raw']}'"
            )
            assert data["status"] == "publish", (
                f"Статус поста не совпадает. Ожидалось: 'publish', получено: '{data['status']}'"
            )
        ...
        with allure.step("Проверить что пост сохранён в таблице wp_posts"):
            ...
            assert db_post["post_title"] == title, (
                f"Заголовок поста в БД не совпадает. Ожидалось: '{title}', получено: '{db_post['post_title']}'"
            )
            assert db_post["post_content"] == content, (
                f"Содержимое поста в БД не совпадает. Ожидалось: '{content}', получено: '{db_post['post_content']}'"
            )
    ...
    def test_create_post_invalid_status(self, api_client: APIClient, db_client: DBClient):
        ...
        payload = {"title": title, "content": "Test content", "status": "invalid_status_xyz"}
        ...
        with allure.step("Проверить что POST /wp/v2/posts вернул 400 и код ошибки rest_invalid_param"):
            ...
            assert response_data["code"] == "rest_invalid_param", (
                f"Код ошибки не совпадает. Ожидалось: 'rest_invalid_param', получено: '{response_data['code']}'"
            )
            assert "status" in response_data["data"]["params"], (
                "Параметр 'status' отсутствует в списке невалидных параметров"
            )

        with allure.step("Проверить что запись не появилась в таблице wp_posts"):
            assert db_client.post_exists_with_title(title) is False, (
    ...
    def test_get_post_by_id(self, api_client: APIClient, db_client: DBClient, make_post):
        ...
        with allure.step("Проверить что GET /wp/v2/posts/{id} возвращает актуальные данные поста"):
            ...
            assert response_data["title"]["raw"] == expected_title, (
                f"Заголовок поста не совпадает. Ожидалось: '{expected_title}', получено: '{response_data['title']['raw']}'"
            )
            assert response_data["status"] == expected_status, (
                f"Статус поста не совпадает. Ожидалось: '{expected_status}', получено: '{response_data['status']}'"
            )

        with allure.step("Проверить что данные поста совпадают с записью wp_posts"):
            db_post = db_client.get_post_by_id(post_id)
            assert db_post["post_title"] == expected_title, (
                f"Заголовок поста в БД не совпадает. Ожидалось: '{expected_title}', получено: '{db_post['post_title']}'"
            )
    ...
    def test_get_non_existent_post(self, api_client: APIClient, db_client: DBClient):
        ...
        with allure.step("Проверить что GET /wp/v2/posts/{id} вернул 404 и код rest_post_invalid_id"):
            ...
            assert response_data["code"] == "rest_post_invalid_id", (
                f"Код ошибки не совпадает. Ожидалось: 'rest_post_invalid_id', получено: '{response_data['code']}'"
            )
    ...
    def test_list_posts(self, api_client: APIClient, db_client: DBClient, make_post):
        ...
        with allure.step("Проверить что GET /wp/v2/posts&include=<ids> вернул нужные ID"):
            ...
            assert first_post["id"] in returned_ids, (
                f"ID первого поста ({first_post['id']}) отсутствует в списке возвращенных постов"
            )
            assert second_post["id"] in returned_ids, (
                f"ID второго поста ({second_post['id']}) отсутствует в списке возвращенных постов"
            )
    ...
    def test_update_post(self, api_client: APIClient, db_client: DBClient, make_post):
        ...
        with allure.step("Проверить что POST /wp/v2/posts/{id} вернул обновленные поля"):
            ...
            assert response_data["title"]["raw"] == new_title, (
                f"Заголовок поста не обновлен. Ожидалось: '{new_title}', получено: '{response_data['title']['raw']}'"
            )
            assert response_data["content"]["raw"] == new_content, (
                f"Содержимое поста не обновлено. Ожидалось: '{new_content}', получено: '{response_data['content']['raw']}'"
            )

        with allure.step("Проверить что обновленный пост сохранен в таблице wp_posts"):
            db_post = db_client.get_post_by_id(post_id)
            assert db_post["post_title"] == new_title, (
                f"Заголовок поста в БД не обновлен. Ожидалось: '{new_title}', получено: '{db_post['post_title']}'"
            )
    ...
    def test_delete_post(self, api_client: APIClient, db_client: DBClient, make_post):
        ...
        with allure.step("Проверить что DELETE /wp/v2/posts/{id} вернул подтверждение удаления"):
            ...
            assert response_data["deleted"] is True, (
                f"Флаг удаления не установлен. Ожидалось: True, получено: {response_data['deleted']}"
            )
            assert response_data["previous"]["id"] == post_id, (
                f"ID удаленного поста не совпадает. Ожидалось: {post_id}, получено: {response_data['previous']['id']}"
            )
        ...
        with allure.step("Проверить что удаленный пост отсутствует в таблице wp_posts"):
            assert db_client.post_exists(post_id) is False, (
                f"Пост с ID {post_id} все еще существует в БД после удаления"
            )
